...
def read_unix_proc(proc_unix_path="/proc/net/unix", debug=True):
    ...
    with open(proc_unix_path, 'r') as file_descriptor:
        for line in file_descriptor:
            ...
            match = re.search("([\d+A-Fa-f]+)\:\s+([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\d]+)\s+([.+$]?)", line)
            if match:
...
def read_tcp_proc(proc_tcp_path="/proc/net/tcp", debug=True):
    ...
    with open(proc_tcp_path, 'r') as file_descriptor:
        for line in file_descriptor:
            match = re.search("(\d+)\:\s+([\dA-Fa-f]+)\:([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\:([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\:([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\:([\dA-Fa-f]+)\s+([\dA-Fa-f]+)\s+([\d]+)\s+([\d]+)\s+([\d]+)", line)
            if match:
                ...
                results[inode] = (local_ip, local_port, remote_ip, remote_port, state, rx, rx)
...
def print_details(tcp_socket_map, udp_socket_map, unix_sockets, open_files):
    ...
    for key in tcp_socket_map:
        if len(tcp_socket_map[key])>1:
            print "There are %d connections to %s" % ( len(tcp_socket_map[key]),
                                                       key)

        for entry in tcp_socket_map[key]:
            print "Local %-15s:%6d\tRemote: %15s:%6d\tState: %s (rx: %d, tx:%d)" % entry
    ...
    for key in udp_socket_map:
        if len(udp_socket_map[key])>1:
            print "There are %d connections to %s" % ( len(udp_socket_map[key]),
                                                       key)
