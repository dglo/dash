...
class DAQNewConfigTest(unittest.TestCase):
    def initPDAQHome(self):
        ...
        if not os.path.exists(os.path.join(tstRsrc, 'new_format')):
            self.fail('No "new_format" directory in resource directory ' +
                      tstRsrc)
    ...
    def testNames(self):
        cfgDir = os.path.join(self.initPDAQHome(), "new_format")
    ...
    def testListsSim5(self):
        ...
        cfg = DAQConfigParser.load("simpleConfig", metaDir + "/new_format")
    ...
    def testReplay(self):
        metaDir = self.initPDAQHome()
        cfg = DAQConfigParser.load("replay-ic22-it4", metaDir + "/new_format")
...
class DAQConfigTest(unittest.TestCase):
    def initPDAQHome(self):
        ...
        if not os.path.exists(tstRsrc):
            self.fail('Cannot find test resources')
        if not os.path.exists(os.path.join(tstRsrc, 'old_format')):
            self.fail('No "old_format" directory in resource directory ' +
                      tstRsrc)
        os.environ["PDAQ_HOME"] = tstRsrc
        return tstRsrc
    ...
    def testNames(self):
        cfgDir = os.path.join(self.initPDAQHome(), "old_format")
    ...
    def testListsSim5(self):
        metaDir = self.initPDAQHome()
        cfg = DAQConfigParser.load("simpleConfig", metaDir + "/old_format")
    ...
    def testReplay(self):
        metaDir = self.initPDAQHome()
        cfg = DAQConfigParser.load("replay-ic22-it4", metaDir + "/old_format")
